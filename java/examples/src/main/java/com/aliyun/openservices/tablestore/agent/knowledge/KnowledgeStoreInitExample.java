...
public class KnowledgeStoreInitExample {
    ...
    public void example() {
         ...
         *
         * <li>如果业务是多租户场景，那么开启多租户优化有助于提高性能，增加召回率。</li> <li>如果业务是单租户场景，忽略该参数即可。通常单租户场景，每次查询会查询整个库里的所有数据而不是某一个用户/子知识库/子模块的部分数据。</li>
         *
        ...
        KnowledgeStoreImpl store = KnowledgeStoreImpl.builder()
            ...
            .embeddingDimension(512) // 向量维度必须设置。 通常情况下选择Embedding模型的维度在512~1024之间。太高维度会增加检索时间，带来的边际收益会下降。
