...
class TablestoreExpressionConverter {
    ...
    static com.aliyun.openservices.tablestore.agent.model.filter.Filter convertOperand(Operand operand, Set<String> tenantIds, boolean enableParseTenantIds) {
        if (operand == null) {
            return null;
        }
        if (operand instanceof Expression expression) {
            if (expression.type() == ExpressionType.NOT) {
                ...
                throw Exceptions.illegalArgument("the left value must be Group or Expression when operand type is NOT, but got:%s", expression.left());
            } else if (expression.type() == ExpressionType.AND) {
                return Filters.and(
                    convertOperand(expression.left(), tenantIds, enableParseTenantIds),
                    convertOperand(expression.right(), tenantIds, enableParseTenantIds)
                );
            } else if (expression.type() == ExpressionType.OR) {
                return Filters.or(convertOperand(expression.left(), tenantIds, false), convertOperand(expression.right(), tenantIds, false));
            } else {
                ...
                return parseComparison((Key) expression.left(), (Value) expression.right(), expression, tenantIds, enableParseTenantIds);
            }
        }
    ...
    private static com.aliyun.openservices.tablestore.agent.model.filter.Filter parseComparison(
        Key key,
        Value value,
        Expression exp,
        Set<String> tenantIds,
        boolean enableParseTenantIds
    ) {
        ...
        switch (type) {
            ...
            default:
                throw Exceptions.illegalArgument("unsupported expression type:%s for key:%s, value:%s", type, key.key(), value.value());
        }
    }
    ...
    private static List<Object> parseList(Key key, Value value) {
        ...
        throw Exceptions.illegalArgument("unsupported value type for key:%s, only supports list but got:%s", key.key(), value.value());
    }
