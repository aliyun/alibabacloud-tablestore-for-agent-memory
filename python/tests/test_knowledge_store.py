...
def test_knowledge_store(multi_tenant_knowledge_store: KnowledgeStore, embedding_model: MockEmbedding):
    ...
    assert document.text_content == document_get.text_content
    assert document.vector_content == document_get.vector_content
    ...
    document_updated.vector_content = None
    document_updated.text_content = None
...
def test_single_knowledge_search(single_knowledge_store: KnowledgeStore, embedding_model: MockEmbedding):
    ...
    documents, next_token = knowledge_store.search_documents(limit=100)
    assert len(documents) == total_document_count
    print(len(documents))
    assert next_token is None, next_token

    documents, next_token = knowledge_store.search_documents(metadata_filter=Filters.all(), limit=100)
    assert next_token is None, next_token
    assert len(documents) == total_document_count
    hit = documents[0]
    print("hit", hit)
    ...
    documents, next_token = knowledge_store.search_documents(metadata_filter=Filters.eq("user_id", "1"), limit=100)
    assert len(documents) == user_id_1_count
    documents, next_token = knowledge_store.search_documents(metadata_filter=Filters.eq("user_id", "1"), limit=3)
    ...
    print(next_token)
    while True:
        sub_docs, next_token = knowledge_store.search_documents(
    ...
    documents, next_token = knowledge_store.search_documents(
    ...
    next_token = None
    while True:
        sub_documents, next_token = knowledge_store.full_text_search(
            query="abc",
            metadata_filter=Filters.eq("user_id", "1"),
            limit=2,
            next_token=next_token,
            meta_data_to_get=["meta_boolean", "meta_string", "text_content", "user_id"],
        )
    ...
    assert all(
        ...
        and hit.document.text_content is not None
    ...
    assert all(
        "abc" in hit.document.text_content for hit in document_hits
    ), f"Not all elements assert true, detail is:{[hit.document.text_content for hit in document_hits]}"

    document_hits = []
    next_token = None
    while True:
        sub_documents, next_token = knowledge_store.vector_search(
...
def test_multi_tenant_knowledge_search(multi_tenant_knowledge_store: KnowledgeStore, embedding_model: MockEmbedding):
    ...
    documents, next_token = knowledge_store.search_documents(limit=100, tenant_id="1")
    assert len(documents) == user_id_1_count
    print(len(documents))
    assert next_token is None, next_token
    ...
    documents, next_token = knowledge_store.search_documents(metadata_filter=Filters.all(), limit=100, tenant_id="1")
    assert next_token is None, next_token
    assert len(documents) == user_id_1_count

    documents, next_token = knowledge_store.search_documents(tenant_id="1", limit=100)
    assert len(documents) == user_id_1_count
    documents, next_token = knowledge_store.search_documents(tenant_id="1", limit=3)
    assert len(documents) == 3
    assert all(
        hit.document.metadata["user_id"] == "1" for hit in documents
    ), f"Not all elements assert true, detail is:{[hit.document.metadata['user_id'] for hit in documents]}"
    ...
    print(next_token)
    while True:
        sub_docs, next_token = knowledge_store.search_documents(tenant_id="1", limit=3, next_token=next_token)
        documents.extend(sub_docs)
        if next_token is None:
            break
    print(len(documents))
    ...
    document_hits, _ = knowledge_store.search_documents(limit=10)
    assert all(
    ...
    assert all(
        ...
        and document_hit.document.text_content is not None
