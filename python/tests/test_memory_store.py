...
def test_get_sessions(memory_store, tablestore_client):
    ...
    token = None
    while True:
        response = memory_store.list_recent_sessions_paginated(user_id="1", page_size=3, next_token=token)
        sub_sessions, token = (response.hits, response.next_token)
...
def test_get_message(memory_store):
    ...
    all_message = list(memory_store.list_all_messages())
    ...
    messages = list(memory_store.list_messages(session_id="1", max_count=5))
    ...
    for test_times in range(20):
        ...
        messages = list(
            memory_store.list_messages(session_id="1", inclusive_start_create_time=desc_start, order=Order.DESC)
        )
        assert all(
            x.create_time <= desc_start for x in messages
        ), f"Not all elements assert true, detail is:{[x.create_time for x in messages]}"
        messages = list(
            memory_store.list_messages(session_id="1", inclusive_end_create_time=desc_end, order=Order.DESC)
        )
        assert all(
        ...
        messages = list(
            memory_store.list_messages(session_id="1", inclusive_start_create_time=asc_start, order=Order.ASC)
        )
        assert all(
            asc_start <= x.create_time for x in messages
        ), f"Not all elements assert true, detail is:{[x.create_time for x in messages]}"
        messages = list(memory_store.list_messages(session_id="1", inclusive_end_create_time=asc_end, order=Order.ASC))
        assert all(
    ...
    messages = list(memory_store.list_messages(session_id="1", metadata_filter=Filters.eq("meta_boolean", True)))
    assert len(messages) == session_id_meta_boolean_true_count

    messages = list(memory_store.list_messages(session_id="1", metadata_filter=Filters.not_eq("meta_boolean", False)))
    assert len(messages) == session_id_meta_boolean_true_count
    ...
    token = None
    while True:
        response = memory_store.list_messages_paginated(session_id="1", page_size=3, next_token=token)
        sub_message, token = (response.hits, response.next_token)
        messages_paginated.extend(sub_message)
        if token is None:
            break
    ...
    token = None
    while True:
        response = memory_store.list_messages_paginated(session_id="1", page_size=3, next_token=token, order=Order.ASC)
        sub_message, token = (response.hits, response.next_token)
        messages_paginated.extend(sub_message)
        if token is None:
            break
    ...
    test_times = random.randint(10, 60)
    for _ in range(test_times):
        asc_start = random.randint(1, 50)
        asc_end = random.randint(50, 95)
        messages = memory_store.list_messages_paginated(
            session_id="1",
            inclusive_start_create_time=asc_start,
            inclusive_end_create_time=asc_end,
            order=Order.ASC,
        ...
        assert all(
            asc_start <= x.create_time <= asc_end for x in messages.hits
        ), f"Not all elements assert true, detail is:{[x.create_time for x in messages.hits]}"

        desc_start = random.randint(50, 95)
        desc_end = random.randint(1, 50)
        messages = memory_store.list_messages_paginated(
            session_id="1",
            inclusive_start_create_time=desc_start,
        ...
        assert all(
            desc_end <= x.create_time <= desc_start for x in messages.hits
        ), f"Not all elements assert true, detail is:{[x.create_time for x in messages.hits]}"
    ...
    all_messages = list(memory_store.list_all_messages())
...
def test_pydantic(memory_store):
    ...
    assert memory_store is not None
    memory_store.list_messages(session_id="1", batch_size=1)
    with pytest.raises(ValidationError):
        memory_store.list_messages(session_id="1", batch_size=6000)
...
def test_session_search_index(memory_store):
    ...
    response = memory_store.search_sessions(limit=100)
    sessions, next_token = (response.hits, response.next_token)
    assert len(sessions) == total_count
    print(len(sessions))
    assert next_token is None, next_token

    response = memory_store.search_sessions(metadata_filter=Filters.all(), limit=100)
    sessions, next_token = (response.hits, response.next_token)
    assert next_token is None, next_token
    assert len(sessions) == total_count
    session = sessions[0]
    ...
    response = memory_store.search_sessions(metadata_filter=Filters.eq("user_id", "1"), limit=100)
    sessions, next_token = (response.hits, response.next_token)
    assert len(sessions) == user_id_1_count
    response = memory_store.search_sessions(metadata_filter=Filters.eq("user_id", "1"), limit=5)
    sessions, next_token = (response.hits, response.next_token)
    ...
    while True:
        ...
        sub_sessions, next_token = (response.hits, response.next_token)
...
def test_message_search_index(memory_store):
    ...
    response = memory_store.search_messages(limit=100)
    messages, next_token = (response.hits, response.next_token)
    assert len(messages) == total_count
    print(len(messages))
    assert next_token is None, next_token

    response = memory_store.search_messages(metadata_filter=Filters.all(), limit=100)
    messages, next_token = (response.hits, response.next_token)
    assert next_token is None, next_token
    assert len(messages) == total_count
    message = messages[0]
    ...
    response = memory_store.search_messages(metadata_filter=Filters.eq("session_id", "1"), limit=100)
    messages, next_token = (response.hits, response.next_token)
    assert len(messages) == session_id_1_count
    response = memory_store.search_messages(metadata_filter=Filters.eq("session_id", "1"), limit=5)
    messages, next_token = (response.hits, response.next_token)
    ...
    while True:
        ...
        sub_messages, next_token = (response.hits, response.next_token)
