...
class KnowledgeStore(BaseKnowledgeStore):
    ...
    def __init__(
            ...
            text_field: Optional[str] = "text_content",
            vector_field: Optional[str] = "vector_content",
    ...
    def put_document(self, document: Document) -> None:
        ...
        if document.text_content:
            attribute_columns.append((self._text_field, document.text_content))
        if document.vector_content:
            attribute_columns.append((self._vector_field, json.dumps(document.vector_content)))
    ...
    def delete_document_by_tenant(self, tenant_id: str) -> None:
        next_token = None
        while True:
            document_hits, next_token = self.search_documents(
                tenant_id=tenant_id,
                limit=1000,
    ...
    def get_document(self, document_id: str, tenant_id: Optional[str] = None) -> Optional[Document]:
        ...
        document = TablestoreHelper.row_to_document(row, self._text_field, self._vector_field)
        return document
    ...
    def get_documents(self, document_id_list: List[str], tenant_id: Optional[str] = None) -> List[Optional[Document]]:
        ...
        for start in range(0, total, batch_size):
            ...
            for i in range(len(table_result)):
                item = table_result[i]
                if item.is_ok:
                    document = TablestoreHelper.row_to_document(item.row, self._text_field, self._vector_field)
    ...
    def get_all_documents(self) -> Iterator[Document]:
        iterator = TablestoreHelper.GetRangeIterator(
            ...
            translate_function=lambda row: TablestoreHelper.row_to_document(row, self._text_field, self._vector_field),
    ...
    def search_documents(self,
        ...
        try:
            ...
            raise e
        hits, next_token = TablestoreHelper.search_response_to_document(search_response=search_response, text_field=self._text_field, vector_field=self._vector_field)
    ...
    def _create_search_index(self):
        ...
        self._search_index_schema = TablestoreHelper.add_schema(
            tablestore.FieldSchema(self._vector_field,
    ...
    def _check_vector_dimension(self, document: Optional[Document] = None) -> None:
        if document is None:
            return
        if document.vector_content is None:
            return
        if len(document.vector_content) != self._vector_dimension:
            raise ValueError(f"document's vector_content embedding length:{len(document.vector_content)} is not the same as the knowledge store dimension:{self._vector_dimension}")
