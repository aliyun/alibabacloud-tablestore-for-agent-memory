...
from tablestore import (
    ...
    SecondaryIndexMeta,
    SecondaryIndexType,
    SingleColumnCondition, Query, MatchAllQuery, BoolQuery, )
...
class TablestoreHelper:
    ...
    @staticmethod
    def row_to_document(row: Optional[tablestore.Row], text_field: str, vector_field: str) -> Optional[Document]:
        if row is None:
            return None
    ...
    @staticmethod
    def search_response_to_document(search_response: tablestore.SearchResponse, text_field: str, vector_field: str) -> (List[DocumentHit], Optional[str]):
        hits = []
        next_token = search_response.next_token
        if next_token:
            next_token = base64.b64encode(next_token).decode('utf-8')
        ...
        for hit in search_response.search_hits:
            ...
            for col in row[1]:
                ...
                if key == vector_field:
                    ...
                    continue
                meta_data[key] = val
            document = Document(document_id=document_id, tenant_id=tenant_id, text_content=text_content, vector_content=vector_content, metadata=meta_data)
